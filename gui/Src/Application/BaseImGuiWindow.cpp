...
void BaseImGuiWindow::Menu( const WindowMenuItems& items )
{
    ...
    // Before displaying the menu, declare the chosen menu action
    WindowMenuItemAction menuItemAction;
    ...
    for (int i = 0; i < numberOfButtons; i++)
    {
        if (i < items.size()) // active button
        {
            ...
            auto textSize = ImGui::CalcTextSize( items[i].first.c_str() );

            if (textSize.x + 10 > menuButtonSize.x)
            {
                adjustedText = items[i].first;
                std::replace( adjustedText.begin(), adjustedText.end(), ' ', '\n' );
            }

            if (ImGui::Button( adjustedText.empty() ? items[i].first.c_str() : adjustedText.c_str(), menuButtonSize ))
            {
                m_bDisplayMainMenu = false;
                menuItemAction = items[i].second;
            }
