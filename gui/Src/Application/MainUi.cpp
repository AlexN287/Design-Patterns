...
{
    if (m_currentView)
        m_currentView->GetViewModel()->OnResize( m_windowInfo.width, m_windowInfo.height, m_windowInfo.pixelRatio );
}
...
{
    m_currentView->GetViewModel()->OnScroll( delta * 1000, Xy( pos.x, pos.y ) );
}
...
void MainUi::OnNonUiTouch( SDL_EventType event, int64_t fingerId, int x, int y )
{
    ...
    case SDL_FINGERDOWN:
    {
        ...
        m_currentView->GetViewModel()->OnTouch( ETouchEvent::TE_Down, fingerId, Xy( x, y ) );
    ...
    case SDL_FINGERUP:
    {
        if (m_bMouseClicked || fingerId)
            m_currentView->GetViewModel()->OnTouch( ETouchEvent::TE_Up, fingerId, Xy( x, y ) );
    ...
    {
        if (m_bMouseClicked)
            m_currentView->GetViewModel()->OnTouch( ETouchEvent::TE_Move, fingerId, Xy( x, y ) );
        break;
    }
    }
}
...
void MainUi::OnNonUiKey( SDL_Scancode code, SDL_EventType action )
{
    ...
    m_currentView->GetViewModel()->OnKey( key, act );
}
